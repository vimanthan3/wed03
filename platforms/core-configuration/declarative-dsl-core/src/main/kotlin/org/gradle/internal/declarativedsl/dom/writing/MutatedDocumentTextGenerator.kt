...
class MutatedDocumentTextGenerator {
    ...
    fun generateText(
        ...
        replaceValue: (ownerTag: ChildTag.ValueNodeTag) -> DeclarativeDocument.ValueNode? = { null },
    ): String {
            ...
            if (!removeNodeIf(ownerTag)) {
                when (ownerTag) {
                    ...
                    is ChildTag.CallArgument -> {
                        (ownerTag as? ChildTag.ValueNodeTag)?.let(replaceValue)?.let { insertSyntheticValue(textBuilder, it, textTreeNode.lineRange.last) }
                            ?: run {

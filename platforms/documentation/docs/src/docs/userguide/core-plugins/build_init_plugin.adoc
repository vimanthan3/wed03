...
If the `--overwrite` option is provided, Gradle overwrites existing files in the directory where the `init` task runs. By default, `init` fails if Gradle finds any files in the directory.

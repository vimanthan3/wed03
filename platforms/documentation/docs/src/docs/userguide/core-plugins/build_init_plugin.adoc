...
If the `--overwrite` option is provided, Gradle will have permission to overwrite any existing files in the directory where the `init` task is run. Otherwise, `init` will fail if any files are found in the directory.

...
When copying files, Gradle will copy the contents of a symlink, rather than the symlink itself by default.
If you want to copy the symlink as is, you can use the `linkStrategy` property on the `CopySpec` to control how symlinks are handled.
...
The available values for `linkStrategy` are:

* `LinkStrategy.FOLLOW` (default): treat any symlink as a file it points to.
* `LinkStrategy.ERROR`: fail the copy if a symlink is encountered.
* `LinkStrategy.PRESERVE_ALL`: copy any symlink "as is".
* `LinkStrategy.PRESERVE_RELATIVE` copy a symlink "as is" if it's relative and points to a file within copy root, otherwise fail the copy.
...
Since an archive is treated as a self-contained file system, there is limited support for some scenarios.
To be specific, no link would be followed if it points outside the archive.
...
Since they are essentially a stream of data and do not support random access, it is not possible to follow a symlink to a file that is not yet processed.

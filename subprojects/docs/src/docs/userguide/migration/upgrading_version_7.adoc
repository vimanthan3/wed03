...
=== Legacy `base` plugin convention configuration

The `base` plugin defined a deprecated convention of type `BasePluginConvention`. In Gradle 8.0, the convention is removed and completely replaced by the `base` extension of type `BasePluginExtension`.
